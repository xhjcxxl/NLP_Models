...
from model_mulit_CNN import MultiCNNText
...
def parser_set():
    ...
    parser.add_argument('-batch-size', type=int, default=16, help='训练中 一个批量的数据个数[默认: 16]')
    ...
    parser.add_argument('-embed-dim', type=int, default=200, help='Glove词向量维度[默认: 200]')
...
def mr_multi(text_field, label_field, static_text_field, static_label_field, **kwargs):
    ...
    if not os.path.exists(cache):
        print("不存在此目录,创建此目录")
        os.mkdir(cache)
    vectors = Vectors(name='.vector_cache/glove.6B/glove.6B.200d.txt', cache=cache)
...
if __name__ == '__main__':
    ...
    if args.predict is not None:
        ...
        print("开始预测模型")
        label = train_data.predict(args.predict, cnn, args.text_field, args.label_field, False)
